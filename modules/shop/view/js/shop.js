function load_filters() {
    $.ajax({
        type: "POST",
        dataType: "json",
        url: friendlyURL("?module=shop&op=filters")
    })
    .done(function( data ) {
        if(data.length==0){
            $(".shop__details").empty();
...
function load_list_products(items_page = 12, total_prod = 0) {
    ...
    if (filters != false) {
        var url = friendlyURL("?module=shop&op=list_filters_products");
        ...
        var url = friendlyURL("?module=shop&op=list_products");
    }
    ...
    $.ajax({
        ...
        dataType: "json",
        url: url,
        data: {items_page: items_page, total_prod: total_prod, filters: filters}
    })
    .done(function( data ) {
        console.log(data);
        if(data.length==0){
            ...
            for (row in data) {
                $("#list_product").append(
                    ...
                    "<div class='list__imges' id='"+ data[row].codigo_producto +"'> <img class='list__img' src='http://localhost/Framework_PHP_OO_MVC/"+ data[row].images +"'> </div>" +
...
function load_pagination(){
    ...
    if (localStorage.getItem('filters')) {
        var url = friendlyURL("?module=shop&op=pagination_filters");
    }else {
        var url = friendlyURL("?module=shop&op=pagination");
    }
...
function load_details() {
    ajaxPromise(friendlyURL("?module=shop&op=details"), 'GET', 'JSON', {id: localStorage.getItem('codigo_producto')})
    .then(function(data) { 
        $.ajax({
            type: "POST",
        ...
        $(".shop__details").append(
            "<div class='container py-xl-5 py-lg-3'> <div class='row'> <div class='col-lg-6 left-wthree-img text-right'> <img src='http://localhost/website/" + data[0].images +
            ...
            " <a href='index.php?module=shop&op=view' class='btn button-style button-style-2 mt-sm-5 mt-4'>Volver</a> </div> </div> </div>"
        ); 
        load_api();
    }).catch(function() {
        //window.location.href = 'index.php?module=error503'
    });  
...
function load_like(){
    if(localStorage.getItem('token') == null){
        ...
        ajaxPromise(friendlyURL("?module=shop&op=load_like"), 'GET', 'JSON', {user: localStorage.getItem('token')})
        .then(function(data) { 
            console.log(data);
            for (row in data) {
...
function click_like(){
    $(document).on('click','.list__heart',function () {
        if(localStorage.getItem('token') == undefined){
            ...
            ajaxPromise(friendlyURL("?module=shop&op=click_like"), 'GET', 'JSON', {id: this.getAttribute('id'), user: localStorage.getItem('token')})
            .then(function(data) { 
                console.log(data);
            }).catch(function(data) {
                console.log(data);
